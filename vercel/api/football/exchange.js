...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT')    return handleBuy(req, res, auth);
...
// ---------------------------------------------------------------------------
// PUT /api/football/exchange  { listingId }
// Buy a listing. Merged from exchange/buy.js to stay within Vercel function limit.
// ---------------------------------------------------------------------------
async function handleBuy(req, res, auth) {
  ...
  try {
    ...
    if (deductFromBonus > 0) {
      await client.query(
        `UPDATE sq.football_profiles SET trade_rating_balance = trade_rating_balance - $1 WHERE user_id = $2`,
    ...
    await client.query(
      `UPDATE sq.football_exchange_listings
       SET status = 'sold', buyer_user_id = $1, completed_at = NOW()
       WHERE id::text = $2`,
      [auth.userId, listingId]
    );
    ...
    const acquiredCardResult = await client.query(
      `SELECT ${toCardRowSelect('uc')}
        ...
        LEFT JOIN sq.football_clubs c ON c.id = p.club_id
        LEFT JOIN sq.football_leagues l ON l.id = p.league_id
        WHERE uc.id::text = $1 AND uc.user_id = $2
        LIMIT 1`,
    ...
    return res.status(200).json({
      success: true,
      data: { listingId, acquiredCard: acquiredCardResult.rows[0] ?? null, costPaid: ratingCost },
    });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('PUT /api/football/exchange failed', error);
