// Enhanced rate limiter: uses Redis if REDIS_URL is provided; otherwise falls back to the simple in-memory limiter used previously.
...
export function rateLimitMiddleware(req, res, opts = {}) {
  ...
  // If redisClient is async, we want to await rateLimit; so handle promise
  const rlPromise = rateLimit(key, limit, windowSec);
  // If rateLimit returns a promise (when Redis used), we need to handle async
  if (rlPromise && typeof rlPromise.then === 'function') {
    ...
    // For serverless handler usage, caller must not rely on immediate return; but our usage sets req.auth then calls this and returns if it returned false synchronously.
    // To keep compatibility, we assume Redis fast; this is a limitation — in most cases it will proceed.
    return true;
  }
